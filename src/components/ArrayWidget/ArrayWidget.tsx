...
function renderFormItem(type: string, setFormData: any, properties?: any) {
  switch (type) {
    case 'string':
      return (
        <Input onChange={ (e: any) => setFormData(e.target.value)} />
      )
    case 'boolean':
      return (
        <Switch onChange={checked => { console.log('checked',checked); setFormData(checked) } } />
      )
    case 'number':
      return (
        <InputNumber size="small" onChange={val => setFormData(val)} />
      )
    case 'object':
      return (
        <div>
          {Object.keys(properties).map((key: any, index: number) => {
            ...
            <label className="fr-label-title no-colon ml2 w-100" key={key}>
              <span className="flex-none" style={{ marginTop: '5px' }}>{property.title}</span>
...
export default function ArrayWidget(props: any) {
  const {
    name,
    schema,
    onChange
